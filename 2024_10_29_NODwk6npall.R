...
fac_DEseq <- cbind(grouped, batch) # Combine groups and batch 
 ...
 # assay(vsd) <- mat
  ...
  sample.diff(r_counts, samples) # Likeness of samples, function in 30AugRRU
...
heatmap.2(as.matrix(EN7.2), scale="row", col=coul, key= T, xlab="", ylab="", 
          margins=c(7,7), ColSideColors=colind, trace="none", key.title=NA, 
          key.ylab=NA, keysize=0.8, dendrogram="both")
ggbiplot(prcomp(t(EN7.2), scale.=T), ellipse=T, groups=names(colind), var.axes=F, 
         var.scale=1, circle=T) + 
  theme_classic() + scale_color_manual(name="Group", values=ecolor)
...
#FlexiDEG
groups = groupedt.5t1 
t.5t1samples = colnames(t.5t1)
metadata = cbind(t.5t1samples, groups)
colnames(metadata) = c("Samples", "Group")
metadata <- as.data.frame(metadata)
pdf(file= "NOD t.5t1, bc2, 50x.pdf" )
NODt.5t1flexi <- flexiDEG.function4(t.5t1, metadata, validation_option = 2)
ENt1.1 <- na.omit(t.5t1[unique(rownames(t.5t1)[as_vector(NODt.5t1flexi[[1]])]), ]) 
ENt1.2 <- na.omit(t.5t1[unique(rownames(t.5t1)[as_vector(NODt.5t1flexi[[2]])]), ]) 
ENt1.3 <- na.omit(t.5t1[unique(rownames(t.5t1)[as_vector(NODt.5t1flexi[[3]])]), ]) 

heatmap.2(as.matrix(ENt1.1), scale="row", col=coul, key= T, xlab="", ylab="", 
          margins=c(7,7), ColSideColors=colind, trace="none", key.title=NA, 
          key.ylab=NA, keysize=0.8, dendrogram="both")
ggbiplot(prcomp(t(ENt1.1), scale.=T), ellipse=T, groups=names(colind), var.axes=F, 
         var.scale=1, circle=T) + 
  theme_classic() + scale_color_manual(name="Group", values=ecolor)

heatmap.2(as.matrix(ENt1.2), scale="row", col=coul, key= T, xlab="", ylab="", 
          margins=c(7,7), ColSideColors=colind, trace="none", key.title=NA, 
          key.ylab=NA, keysize=0.8, dendrogram="both")
ggbiplot(prcomp(t(ENt1.2), scale.=T), ellipse=T, groups=names(colind), var.axes=F, 
         var.scale=1, circle=T) + 
  theme_classic() + scale_color_manual(name="Group", values=ecolor)

heatmap.2(as.matrix(ENt1.3), scale="row", col=coul, key= T, xlab="", ylab="", 
          margins=c(7,7), ColSideColors=colind, trace="none", key.title=NA, 
          key.ylab=NA, keysize=0.8, dendrogram="both")
ggbiplot(prcomp(t(ENt1.3), scale.=T), ellipse=T, groups=names(colind), var.axes=F, 
         var.scale=1, circle=T) + 
  theme_classic() + scale_color_manual(name="Group", values=ecolor)
...
colnames(metadata) = c("Samples", "Group")
metadata <- as.data.frame(metadata)
pdf(file= "NOD week 6, NS all, bc2, 50x,2.pdf" )
NODwk6flexi <- flexiDEG.function4(wk6pnpall, metadata, validation_option = 2)
EN7.3 <- na.omit(wk6pnpall[unique(rownames(wk6pnpall)[as_vector(NODwk6flexi[[1]])]), ]) 
EN8.3 <- na.omit(wk6pnpall[unique(rownames(wk6pnpall)[as_vector(NODwk6flexi[[2]])]), ]) 
EN9.3 <- na.omit(wk6pnpall[unique(rownames(wk6pnpall)[as_vector(NODwk6flexi[[3]])]), ]) 

heatmap.2(as.matrix(EN7.3), scale="row", col=coul, key= T, xlab="", ylab="", 
          margins=c(7,7), ColSideColors=colind, trace="none", key.title=NA, 
          key.ylab=NA, keysize=0.8, dendrogram="both")
ggbiplot(prcomp(t(EN7.3), scale.=T), ellipse=T, groups=names(colind), var.axes=F, 
         var.scale=1, circle=T) + 
  theme_classic() + scale_color_manual(name="Group", values=ecolor)

heatmap.2(as.matrix(EN8.3), scale="row", col=coul, key= T, xlab="", ylab="", 
          margins=c(7,7), ColSideColors=colind, trace="none", key.title=NA, 
          key.ylab=NA, keysize=0.8, dendrogram="both")
...
#Simple EN, NOD wk 6 vs NS all timepoints
  xfactors <- cbind.data.frame(groupswk6all) 
samplesbc = colnames(wk6pnpall)
  rownames(xfactors) <- samplesbc
  dataset <- t(wk6pnpall) # Load the data. Should it be raw or normalized!?!?!?!
  dataset <- as.data.frame(cbind(xfactors, dataset)) # Add factor as the first column
  dataset <- dataset[complete.cases(dataset), ] # For cases without missed items
  set.seed(123) # Split the data into training and test set
  training.samples <- dataset$groupswk6all %>% createDataPartition(p = 1, list = F)
  train.data  <- dataset[training.samples, ]
  test.data <- dataset[-training.samples, ]
  x <- model.matrix(groupswk6all~., train.data)[,-1] # Predictor variables; need reduced gene set to run, runs w/ 9814
  y <- train.data$groupswk6all # Outcome variable
  genes_EN <- binom_EN( x, y, 100) # number following multinom is number of groups
NODwk6vNSall = genes_EN
batchcorr_counts = wk6pnpall
  ...
  n_EN.05 <- na.omit(batchcorr_counts[g_EN.05, ])
  n_EN0 <- na.omit(batchcorr_counts[g_EN0, ])
pdf(file= "NOD wk6, NS all, bc2.pdf" )
  heatmap.2(as.matrix(n_EN1), scale = "row", col = coul, key = T, 
            xlab = "Sample", ylab="", # density.info="none",
            margins = c(7, 7), ColSideColors = colind, trace="none", # Colv = "Na",
            key.title=NA, key.ylab=NA, keysize = 0.8, dendrogram = "column")
